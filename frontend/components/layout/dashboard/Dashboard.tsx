...
function EarningsChart() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full">
      ...
      {months.map((m, i) => (
        <text key={m} x={x(i)} y={h - 5} textAnchor="middle"
...
export default function Dashboard() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 px-8 py-8 space-y-7">
      ...
      <div className="flex items-start justify-between">
        ...
        <button className="flex items-center gap-2.5 bg-white border border-slate-200
      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        <div className="col-span-12 lg:col-span-5 grid grid-cols-2 gap-5">
          {STATS.map(({ icon: Icon, color, label, value, trend, up }) => (
            <div key={label} className="bg-white border border-slate-200 rounded-2xl p-5
              flex flex-col gap-4 hover:shadow-md hover:border-slate-300
        ...
        <div className="col-span-12 lg:col-span-7 bg-white border border-slate-200
          rounded-2xl p-8 flex gap-6 items-center overflow-hidden relative shadow-sm">
          ...
          <div className="flex-1 relative z-10">
            <div className="flex -space-x-2 mb-5">
              {["bg-orange-400","bg-blue-400","bg-slate-400","bg-emerald-400"].map((c, i) => (
                <div key={i} className={`w-10 h-10 rounded-full ${c}
        ...
        <div className="col-span-12 lg:col-span-5 bg-white border border-slate-200
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-3">
              <p className="text-3xl font-bold text-slate-800">$295.7k</p>
              <span className="text-xs bg-emerald-50 text-emerald-600 px-2.5 py-1
                rounded-full font-semibold border border-emerald-100">
                +2.7%
              </span>
            </div>
          </div>
            ...
            {["BNP Core","Bundle","BNP Nest"].map((l, i) => (
              <div key={l} className="flex items-center gap-1.5">
                <div className={`w-2 h-2 rounded-full
                  ${["bg-emerald-500","bg-red-400","bg-violet-500"][i]}`} />
