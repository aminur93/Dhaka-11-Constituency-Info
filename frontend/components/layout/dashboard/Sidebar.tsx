  ...
  return (
    <>
      ...
      >
        {/* ── Logo ── */}
        <div className={`
          flex items-center h-[68px] shrink-0
          border-b border-white/[0.08]
          ${mini ? "justify-center px-4" : "px-5"}
        `}>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.label}
                      className="relative"
                      onMouseEnter={() => mini && setHovered(item.label)}
                      onMouseLeave={() => mini && setHovered(null)}
                    >
                      {/* Direct link */}
                      {isDirect ? (
                        <Link
                        ...
                        /* Button with children or disabled */
                        <button
                          disabled={disabled}
                          onClick={() => hasKids && !mini && toggle(item.label)}
                          className={`
                            relative w-full flex items-center gap-3
                            px-3 py-[11px] rounded-xl
                            text-[13px] font-medium
                      ...
                      {/* Mini flyout */}
                      {mini && hasKids && hovered === item.label && (
                        <div className="absolute left-full top-0 ml-3 w-52
                          ...
                          <p className="text-[10px] font-bold text-zinc-600 uppercase
                            tracking-widest px-4 pt-1 pb-2 select-none">
                            {item.label}
                          </p>
          ...
          {mini ? (
            ...
            <div className="flex items-center gap-3 px-3 py-3 rounded-xl
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-zinc-200 truncate leading-none mb-1">
                  John Doe
                </p>
                <p className="text-[11px] text-zinc-600 truncate leading-none">
                  john@example.com
                </p>
              </div>
              <Settings className="w-4 h-4 text-zinc-700 shrink-0
                group-hover:text-zinc-400 group-hover:rotate-45
      ...
      {/* ── Toggle button — between sidebar and header ── */}
      <button
        onClick={() => setMini((v) => !v)}
        className={`
          fixed top-[34px] -translate-y-1/2 z-50
      ...
      {/* Spacer */}
      <div className={`shrink-0 transition-all duration-300 ${mini ? "w-[72px]" : "w-[260px]"}`} />
    </>
  );
}